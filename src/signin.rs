...
use crate::util::{dig, format_credit, is_true_or_one, value_string, value_truthy};
...
impl SigninService {
    pub fn new(api: BillingApi) -> Self { Self { api } }

    pub async fn raw_status(&self) -> HttpResult { self.api.status().await }
    pub async fn raw_claim(&self) -> HttpResult { self.api.claim().await }

    pub async fn run(&self) -> ServiceRun {
        let status_result = self.api.status().await;
        if status_result.code == CODE_BUDGET_OUT {
            return ServiceRun { code: 1, out: json!({
                "result": "TIMEOUT",
                "report": "已达本次运行时间预算，签到跳过，下次自动重试"
            })};
        }
        if status_result.code == CODE_NO_NETWORK {
            let error = dig(&status_result.body, "error").and_then(Value::as_str).unwrap_or("");
            return ServiceRun { code: 1, out: json!({
                "result": "NETWORK",
        ...
        if !status_result.is_success() {
            return ServiceRun { code: 1, out: json!({
                "result": "ERROR",
                "report": format!("签到接口返回异常（HTTP {}），请重新登录客户端或稍后重试", status_result.code),
        ...
        if matches!(dig(&status, "active"), Some(Value::Bool(false))) {
            let name = value_string(dig(&status, "activity_name"));
            let report = match name {
                Some(name) => format!("签到活动未开启（{name}）"),
                None => "签到活动未开启".to_string(),
            };
            return ServiceRun { code: 0, out: json!({"result":"INACTIVE","report":report,"active":false}) };
        }

        if is_true_or_one(dig(&status, "today_checked_in")) {
            return ServiceRun { code: 0, out: already_report(&status, None) };
        }

        let claim = self.api.claim().await;
        if matches!(claim.code, CODE_NO_NETWORK | CODE_BUDGET_OUT) {
            let error = dig(&claim.body, "error").and_then(Value::as_str).unwrap_or("");
            let result = if claim.code == CODE_NO_NETWORK { "NETWORK" } else { "TIMEOUT" };
            return ServiceRun { code: 1, out: json!({
                "result": result,
        ...
        if let Some(credit) = dig(&claim.body, "credit").cloned() {
            ...
            let streak_days = dig(&fresh, "streak_days").cloned().or_else(|| dig(&status, "streak_days").cloned());
            ...
            let bonus = if value_truthy(is_streak_day.as_ref()) { "，且为连签奖励日" } else { "" };
            let cumulative = total_credits.as_ref().map(|v| format!("，累计 {} 积分", format_credit(v))).unwrap_or_default();
            let streak = match streak_days.as_ref() {
                Some(days) => format!("（连续 {} 天{}）", display_value(days), cumulative),
                None if !cumulative.is_empty() => format!("（{}）", cumulative.trim_start_matches('，')),
                None => String::new(),
            };
            let report = format!("成功领取 {} 积分{}{}", format_credit(&credit), bonus, streak);
            let mut out = Map::new();
            ...
            out.insert("next_streak_day".into(), next_streak_day.unwrap_or(Value::Null));
            return ServiceRun { code: 0, out: Value::Object(out) };
        }

        if claim.body.as_object().is_some_and(|o| o.contains_key("code") || o.contains_key("msg")) {
...
pub fn is_already_checked_in(body: &Value) -> bool {
    ...
    if obj.get("code").and_then(Value::as_i64) == Some(10001) {
        return true;
    }
    obj.get("msg").and_then(Value::as_str).is_some_and(|msg| msg.contains("已签"))
}

pub fn already_report(status: &Value, via: Option<&str>) -> Value {
    let today_credit = dig(status, "today_credit").cloned().or_else(|| dig(status, "daily_credit").cloned());
    ...
    if let Some(v) = today_credit.as_ref() { inner.push(format!("今日 +{}", format_credit(v))); }
    if let Some(v) = streak_days.as_ref() { inner.push(format!("连续 {} 天", display_value(v))); }
    if let Some(v) = total_credits.as_ref() { inner.push(format!("累计 {} 积分", format_credit(v))); }
    let prefix = via.unwrap_or("今日已签过");
    let report = if inner.is_empty() { prefix.to_string() } else { format!("{}（{}）", prefix, inner.join("，")) };
...
fn no_session_with_http(code: i32) -> ServiceRun {
    ServiceRun { code: 1, out: json!({
        "result": "NO_SESSION",
        "report": format!("登录态已失效（HTTP {code}），请重新登录 WorkBuddy 桌面端"),
        "http": code
    })}
}
...
mod tests {
    ...
    #[test]
    fn already_compatibility_contract() {
        assert!(is_already_checked_in(&Value::Null));
        assert!(is_already_checked_in(&json!({"code":10001,"msg":"whatever"})));
        assert!(is_already_checked_in(&json!({"msg":"今天已签到"})));
        assert!(!is_already_checked_in(&json!({"code":10002,"msg":"invalid"})));
    }

    #[test]
    fn already_report_supports_daily_credit_fallback() {
        let out = already_report(&json!({"daily_credit":"100","streak_days":3}), None);
        assert_eq!(out["result"], "ALREADY");
        assert!(out["report"].as_str().unwrap().contains("今日 +100"));
    }
}
